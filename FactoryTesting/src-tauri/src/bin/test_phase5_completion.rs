...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if test_plc_channels.len() != 88 {
        println!("⚠️  警告：期�?8个测试PLC通道，实际加载了{}�?, test_plc_channels.len());
    }
    ...
    if real_channel_definitions.len() != 88 {
        println!("⚠️  警告：期�?8个被测PLC点位，实际加载了{}�?, real_channel_definitions.len());
    }
    ...
    if allocation_result.batches.len() == 2 {
        ...
        if batch1_count == 59 && batch2_count == 29 {
            println!("�?分配结果完美匹配期望�?9+29分布");
        } else {
            println!("⚠️  分配结果: 批次1({}) + 批次2({}) = {}",
                     batch1_count, batch2_count, batch1_count + batch2_count);
        }
    } else {
        println!("⚠️  期望2个批次，实际生成{}个批�?, allocation_result.batches.len());
    }
    ...
    match persistence_service.batch_save_test_instances(&allocation_result.allocated_instances).await {
        Ok(_) => println!("�?批量保存 {} 个实例成�?, allocation_result.allocated_instances.len()),
    ...
    println!("�?查询�?{} 个已保存的批�?, saved_batches.len());
    ...
    if let Some(first_batch) = saved_batches.first() {
        ...
        println!("�?批次 {} 包含 {} 个实�?, first_batch.batch_name, batch_instances.len());
    ...
    println!("\n🎉 第五阶段验证完成！系统核心功能运行正常�?);
    println!("📋 建议下一�?");
    println!("   1. 在浏览器中测试前端界面功�?);
    println!("   2. 验证批次自动分配的前端操�?);
    println!("   3. 验证通道详情查看的前端显�?);
