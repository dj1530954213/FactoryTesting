...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let readings = vec![
        ...
        AnalogReadingPoint {
            ...
            error_percentage: Some(5.0),     // 5%误差，超�?%容忍�?
        },
        AnalogReadingPoint {
            set_percentage: 75.0,
            set_value_eng: 3.0,
            ...
            status: SubTestStatus::Failed,
            error_percentage: Some(4.27),    // 4.27%误差，超�?%容忍�?
        },
        AnalogReadingPoint {
    ...
    let outcome = RawTestOutcome {
        ...
        message: Some("AI硬点测试部分失败�?0%�?5%点误差过�?.to_string()),
        start_time: Utc::now(),
        end_time: Utc::now(),
        readings: Some(readings.clone()),
        // 🔧 关键：即使测试失败，也要保存百分比测试结�?
    ...
    match persistence_service.load_test_instance(&test_instance.instance_id).await? {
        ...
        None => {
            println!("�?无法从数据库加载失败测试的实�?);
        }
    }
    
    println!("\n🎉 失败硬点测试的过程数据保存功能测试完成！");
    println!("�?验证结果：即使测试失败，所有过程数据（0%-100%）都能正确保存到数据�?);
