...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 检查通道定义�?
    println!("\n🔍 检查通道定义�?(channel_point_definitions)...");
    let definitions = channel_point_definition::Entity::find().all(&db).await?;
    println!("📊 通道定义表中共有 {} 条记�?, definitions.len());
    ...
    println!("📊 批次信息表中共有 {} 条记�?, batches.len());
    ...
    println!("📊 测试实例表中共有 {} 条记�?, instances.len());
    ...
    // 总结
    println!("\n📊 数据库状态总结:");
    println!("  通道定义: {} �?, definitions.len());
    println!("  批次信息: {} �?, batches.len());
    println!("  测试实例: {} �?, instances.len());
