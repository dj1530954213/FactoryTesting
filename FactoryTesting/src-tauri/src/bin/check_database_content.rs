...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        match result {
            Ok(_) => {
                ...
                match count_result {
                    ...
                    Err(e) => println!("❌ 表 {} 不存在或查询失败: {}", table, e),
                }
            },
            Err(e) => println!("❌ 表 {} 检查失败: {}", table, e),
        }
    }
    
    // 检查channel_point_definitions表的内容
    println!("\n📊 检查channel_point_definitions表内容...");
    ...
    match count_result {
        Ok(_) => {
            ...
            match sample_result {
                Ok(_) => println!("✅ 成功查询channel_point_definitions表"),
                Err(e) => println!("❌ 查询channel_point_definitions表失败: {}", e),
            }
        },
    ...
    match batch_count_result {
        Ok(_) => println!("✅ 成功查询test_batch_info表"),
        Err(e) => println!("❌ 查询test_batch_info表失败: {}", e),
    }
