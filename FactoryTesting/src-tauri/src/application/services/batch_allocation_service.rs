//! # 批次分配服务 (Batch Allocation Service)
//!
//! ## 业务说明
//! 批次分配服务负责实现智能的通道分配算法，将导入的通道点位定义合理分配到测试批次中
//! 基于原C#项目的分配逻辑重构，支持多种分配策略和智能优化
//!
//! ## 核心功能
//! - **智能分组**: 根据不同策略将通道分组到合理的批次中
//! - **负载均衡**: 确保各批次的测试负载相对均衡
//! - **时间估算**: 预估各批次的测试时间，优化整体效率
//! - **约束处理**: 处理硬件资源约束和业务规则约束
//!
//! ## 分配策略
//! - **ByModuleType**: 按模块类型分组(AI、AO、DI、DO分别成批)
//! - **ByStation**: 按站点分组(同一站点的通道分配到一个批次)
//! - **ByProductModel**: 按产品型号分组(同型号产品的通道一起测试)
//! - **Smart**: 智能分配(综合考虑多种因素的优化算法)
//!
//! ## 分配算法特点
//! - **启发式优化**: 使用启发式算法寻找接近最优的分配方案
//! - **约束满足**: 严格满足硬件资源和业务规则约束
//! - **性能优化**: 最小化总测试时间和资源冲突
//! - **可配置性**: 支持参数调优和策略自定义
//!
//! ## Rust知识点
//! - **算法实现**: 使用Rust实现复杂的分配算法
//! - **性能优化**: 通过HashMap等数据结构优化查找性能
