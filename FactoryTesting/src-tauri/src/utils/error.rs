...
//!
//! ## 业务说明
//! 本模块定义了FAT_TEST工厂测试系统的统一错误处理机制，提供了完整的错误类型系统
//! 通过统一的错误类型，实现了从底层硬件到上层业务的全链路错误处理
//!
//! ## 核心功能
//! ### 1. 错误类型分类
//! - **通信错误**: PLC设备通信、网络连接等相关错误
//! - **数据错误**: 数据验证、序列化、持久化等相关错误
//! - **业务错误**: 测试流程、状态管理等业务逻辑错误
//! - **系统错误**: I/O操作、并发控制、资源管理等系统级错误
//! - **配置错误**: 配置加载、验证、权限等配置相关错误
//!
//! ### 2. 错误处理特性
//! - **结构化信息**: 每个错误都包含详细的上下文信息
//! - **错误链**: 支持错误的层层包装和原因追踪
//! - **序列化支持**: 错误可以序列化为JSON，便于前端显示
//! - **多语言支持**: 支持中文错误消息，便于用户理解
//!
//! ### 3. 错误恢复机制
//! - **自动重试**: 对于临时性错误支持自动重试
//! - **降级处理**: 关键功能失败时提供降级方案
...
//! ## 错误处理策略
//! ### 按错误严重程度分类
//! - **致命错误**: 导致系统无法继续运行，需要立即终止
//! - **严重错误**: 影响核心功能，需要用户介入处理
//! - **警告错误**: 影响部分功能，可以继续运行但需要关注
//! - **信息错误**: 临时性问题，通常可以自动恢复
//!
//! ### 错误传播链路
//! ```
//! 硬件设备错误 → PLC通信错误 → 领域层业务错误 → 
//! 应用层服务错误 → 接口层响应错误 → 前端用户提示
//! ```
//!
//! ## 使用场景
//! - **PLC通信**: 处理设备连接、通信超时、协议错误等
//! - **数据处理**: 处理Excel解析、数据验证、数据库操作错误
//! - **测试执行**: 处理测试流程中的各种异常情况
//! - **用户交互**: 将技术错误转换为用户友好的提示信息
//!
//! ## Rust知识点
//! - **thiserror**: 使用thiserror简化错误类型定义
//! - **Error trait**: 实现标准的Error trait，支持错误链
//! - **类型安全**: 通过类型系统确保错误处理的正确性
//! - **模式匹配**: 使用match表达式进行错误分类处理
...
/// 
/// 业务说明：
/// 封装系统中可能出现的各种错误，提供统一的错误处理机制
/// 支持错误的分类管理、详细信息记录和用户友好的错误展示
/// 
/// 设计特点：
/// - 枚举类型确保错误处理的完整性
/// - 每个错误都包含详细的上下文信息
...
/// 
/// Rust知识点：
/// - thiserror::Error 自动实现Error trait
/// - #[error(...)] 定义错误的显示格式
/// - Serialize/Deserialize 支持错误的序列化
