...
mod tests {
    ...
    async fn test_system_monitor_creation() {
        ...
        let metrics = monitor.get_business_metrics().await;
        assert_eq!(metrics.total_tests_run, 0);
    }
    
    #[tokio::test]
    async fn test_business_metrics_update() {
        let monitor = SystemMonitor::new();
        
        // 模拟测试完成
        monitor.update_business_metrics(
            true,  // test_completed
            true,  // test_success
    ...
    async fn test_health_check() {
        ...
        let health = monitor.perform_health_check("memory").await;
