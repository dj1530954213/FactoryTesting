...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match persistence_service.load_all_channel_definitions().await {
        Ok(definitions) => {
            println!("✅ 成功加载通道定义，共 {} 条记录", definitions.len());
    ...
    match persistence_service.load_all_batch_info().await {
        Ok(batches) => {
            println!("✅ 成功加载批次信息，共 {} 条记录", batches.len());
    ...
    match persistence_service.load_all_batch_info().await {
        Ok(batches) => {
            if batches.is_empty() {
                ...
                for batch in batches.iter().take(3) { // 只检查前3个批次
                    match persistence_service.load_test_instances_by_batch(&batch.batch_id).await {
                        Ok(instances) => {
                            total_instances += instances.len();
                            if !instances.is_empty() {
                                println!("📋 批次 {} 的测试实例 ({} 个):", batch.batch_id, instances.len());
                                for (i, instance) in instances.iter().take(3).enumerate() {
                                    println!("  {}. 实例ID: {}, 定义ID: {}, 状态: {:?}",
                                        i + 1, instance.instance_id, instance.definition_id, instance.overall_status);
                                }
                            }
                        },
                        Err(e) => {
                            println!("❌ 加载批次 {} 的测试实例失败: {}", batch.batch_id, e);
                        }
                    }
                }
                println!("✅ 总共检查了 {} 个测试实例", total_instances);
            }
        },
        Err(e) => {
            println!("❌ 加载批次信息失败，无法检查测试实例: {}", e);
        }
    }
