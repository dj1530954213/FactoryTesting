...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match persistence_service.load_test_instance(&test_instance.instance_id).await? {
        Some(loaded_instance) => {
            ...
            println!("   - 状态: {:?}", loaded_instance.overall_status);
        }
        None => {
            println!("❌ 数据库中未找到测试实例: {}", test_instance.instance_id);
            return Err("数据库保存测试失败".into());
        }
    }
    ...
    match state_manager.update_test_result(test_outcome).await {
        ...
        Err(e) => {
            ...
            match persistence_service.load_all_test_instances().await {
                Ok(all_instances) => {
                    println!("   - 数据库中共有 {} 个测试实例", all_instances.len());
                ...
                Err(e) => {
                    println!("   - 查询所有测试实例失败: {}", e);
                }
            }
            
            return Err(format!("状态更新测试失败: {}", e).into());
        }
    }
    ...
    match persistence_service.load_test_instance(&test_instance.instance_id).await? {
        Some(updated_instance) => {
            println!("✅ 更新后的测试实例状态: {:?}", updated_instance.overall_status);
            
            // 检查子测试结果
            if let Some(hard_point_result) = updated_instance.sub_test_results.get(&SubTestItem::HardPoint) {
                println!("   - 硬点测试状态: {:?}", hard_point_result.status);
                println!("   - 实际值: {:?}", hard_point_result.actual_value);
            }
        }
        None => {
            println!("❌ 更新后未找到测试实例");
            return Err("状态更新验证失败".into());
        }
    }
    ...
    match cached_instance {
        Some(instance) => {
            println!("✅ 内存缓存中找到测试实例: {}", instance.instance_id);
